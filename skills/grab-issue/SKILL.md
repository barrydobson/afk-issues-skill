...
it, review the resulting PR (reworking it if needed), then hand it back to the
human. You do NOT implement it yourself and you do NOT re-derive the
worktree/PR mechanics - the `issue-worker` agent (Agent/Task tool,
`subagent_type: issue-worker`) already owns all of that.
...
**draft**; you review it and, on a PASS, mark it ready. `draft = not yet passed
review`, `ready = passed`. A human is in the room and still does the merge - your
review and ready flag run first, so they only ever merge a reviewed PR. If your
review says NEEDS WORK, the PR stays draft and you rework it (capped at 2 rounds)
before handing back.
...
On a §4 report with a PR, run the **PR review cycle** in the afk-issues skill's
`references/pr-review.md` - CI watch, quick review against the issue,
`ACCEPTANCE_CHECK` gate, verdict, comment, and the rework loop (§4 there, capped
at 2 rounds). It is the same review `afk-issues` runs; a human being in the room
does not change the mechanics. On PASS it marks the PR ready and comments; on
NEEDS WORK it reworks against the same branch (dispatch prompt per
`dispatch-contract.md` §2, same model as step 3) and re-reviews, up to the cap.
...
Report to the user: the PR URL and whether it is **ready** (passed) or **draft**
(reworked to the cap / CI stalled), the branch name, worktree path, the worker's
one-line summary, its `ACCEPTANCE_CHECK` evidence verbatim, and your review
verdict/comment. If the report included `LEARNINGS`, relay it as-is - you don't
triage it yourself; a human is already in the room to judge what's worth
keeping. Then stop. The merge is the user's call.

## Follow-up signals

After handback the user does their own pass over the PR, then tells you what to
do next. Respond to three signals; reuse the worker's modes, never re-implement
them here.

- **"mark it ready" / "it passes"**: `gh pr ready <url>`. Your review already
  readies a PASS - this covers the cases where it didn't: a PR you parked as
  draft (NEEDS WORK after the cap, or CI stalled) that the human overrides, or a
  `gh pr ready` the auto-mode classifier denied earlier.
...
| "I'll skip the review and just hand the draft back" | Run the review cycle first (step 5). It marks a PASS ready and reworks a NEEDS WORK - the same review `afk-issues` runs. |
| "It looks ready, I'll mark it ready without reviewing" | Ready follows a PASS from the review cycle, not a glance. No review, no ready. |
...
| "One more rework round and it'll be there" | Cap at 2. Park it as draft with a comment and hand it to the human. |
...
| "It says tests pass, so it passes" | `ACCEPTANCE_CHECK` needs evidence (test output, a URL), not a restated claim. Missing or vague evidence is an automatic NEEDS WORK into the rework loop. |

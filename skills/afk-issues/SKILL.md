...
When a worker reports a PR, run the **PR review cycle** in
`references/pr-review.md` - CI watch, quick review against the issue,
`ACCEPTANCE_CHECK` gate, verdict, and comment. Two things are specific to this
unattended run and layer on top of it:

- **CI stall (exit 124):** leave the PR draft, comment that CI never settled, and surface it to the human at handoff (step 7). Move on to other batches - a stuck pipeline must not block the run. Don't surface a NEEDS WORK to the human before the rework cap is hit.
- **Learnings triage.** If the report includes `LEARNINGS`, decide per item: discard (noise) or keep (a durable gotcha or insight). For each kept item, pick a recommended destination - **CLAUDE.md** for a fact about the codebase any future contributor needs, **memory** for a fact about this session's preferences, corrections, or ephemeral project state. **Do not ask about them now.** This is an unattended run - a mid-run `AskUserQuestion` would stall the whole loop until the human is back. Accumulate the kept items (with your recommended destination) in your session context and carry them to handoff (step 7). `LEARNINGS` only lives in your session context for this run, so hold onto the kept items until then rather than dropping them.

### 6. Rework loop

NEEDS WORK routes into the rework loop in `references/pr-review.md` §4 (dispatch
prompt per `dispatch-contract.md` §2, same model as step 3, capped at 2 rounds,
pushes to the same branch). After the cap, leave the PR draft with a summary
comment and surface it to the human at handoff (step 7).
